"""Build the complete, read-only Dashboard view from SQLite."""
...
def build_dashboard(
    ...
    """Return the full Dashboard snapshot, with SQLite as its only input."""
    if database is None:
    ...
    daily_rows = [{"date": key, "amount": float(value)} for key, value in sorted(daily.items())]
