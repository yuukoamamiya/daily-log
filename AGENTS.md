...
维护一个可以公开下载、独立运行、可靠保存个人数据的 SQLite 客户端。新增功能必须优先服务通用客户端，不能重新绑定到维护者的私人仓库、GitHub Issue 或旧版命令行工具链。

本仓库只发布通用客户端所需的源码、资源和构建配置，不包含私人数据、数据库、密钥、备份包或旧仓库历史。

## 架构边界

客户端由 `daily_log/` 本地核心、`web/` 界面和 `python -m daily_log` 启动入口组成。客户端不得在默认启动流程中读取仓库数据、README、Git 状态或 GitHub Issue。新用户首次启动必须创建空数据库。

旧版文本数据只有在用户显式提供 `--migrate-from` 时才能导入，而且只能导入一次；迁移源由用户指定，不能默认为当前仓库目录。
...
- API 密钥、WebDAV 和 S3 凭据必须保存在源码仓库外。
...
python -m daily_log --migrate-from <旧版本目录>
```
...
- 安装版使用 `%LOCALAPPDATA%/DailyLog`；绿色版默认使用 EXE 旁边的 `data/`。安装包和绿色版不得包含仓库私人数据、数据库或密钥。
...
- 发布前后都要验证 PortableApps 包结构、启动器冒烟、ZIP 完整性，以及真实 PortableApps Platform 中的安装、升级和数据保留。
...
涉及 UI 时应在隔离用户目录进行真实浏览器操作测试。涉及备份恢复时必须检查恢复前安全副本和路径边界。涉及发布时必须确认产物不含私人数据、密钥、绝对路径和测试数据库。

GitHub Actions 约定：普通提交和 Pull Request 运行测试与 Windows 构建；只有推送匹配 `vMAJOR.MINOR.PATCH` 的标签才创建正式 Release。标签版本必须与 `daily_log/version.py` 中的 `__version__` 一致。Release 至少包含 Windows 安装版、普通绿色版、PortableApps ZIP、PortableApps `.paf.exe` 和 `SHA256SUMS.txt`。

## 文档职责

- `README.md`：面向普通用户的项目介绍、下载和启动方式。
- `SETUP.md`：面向开发者的环境、测试和目录说明。
- `TODO.md`：尚未完成的维护与产品路线，不记录已经完成的流水账。
