...
function todoRows(items = state.data.todos, limit = null, options = {}) {
  ...
  return visible.map((item) => {
    ...
    const reschedule = options.reschedule && due.key === "overdue"
      ...
      : "";
    return `<div class="todo-item ${due.key}"><button class="todo-check" data-complete-todo="${escapeHtml(item.id)}" aria-label="完成待办">${icon("tick")}</button><div class="todo-content"><p>${escapeHtml(item.text)}</p>${meta}${reschedule}</div><div class="row-actions"><button data-edit-item="todo" data-item-id="${escapeHtml(item.id)}">编辑</button><button class="danger" data-delete-item="todo" data-item-id="${escapeHtml(item.id)}">删除</button></div></div>`;
  }).join("");
}

function completedTodoRows(limit = 10) {
  const items = state.data.completedTodos.slice(0, limit);
  if (!items.length) return `<div class="empty-state compact"><p>近期还没有已完成待办</p></div>`;
  return items.map((item) => `<div class="todo-item completed"><button class="todo-check checked" data-restore-todo="${escapeHtml(item.id)}" aria-label="恢复待办">${icon("tick")}</button><div class="todo-content"><p>${escapeHtml(item.text)}</p><div class="todo-meta"><small>${item.completedDate ? `${shortDate(item.completedDate)}完成` : "已完成"}</small>${item.tags.map((tag) => `<span class="tag amber">${escapeHtml(tag)}</span>`).join("")}</div></div><div class="row-actions"><button data-restore-todo="${escapeHtml(item.id)}">恢复</button><button class="danger" data-delete-item="todo" data-item-id="${escapeHtml(item.id)}">删除</button></div></div>`).join("");
}
...
function renderExpenses() {
  const selected = state.data.transactions.filter((item) => item.date.startsWith(state.month));
  return `<div class="view-header"><div><h2>消费记录</h2><p>${monthLabel()}共 ${selected.length} 笔支出</p></div><div style="display:flex;gap:10px">${monthControl()}<button class="primary-button" data-open-composer="transaction">${icon("plus")}记一笔</button></div></div><div class="data-layout"><section class="card"><table class="data-table"><thead><tr><th>项目</th><th>日期</th><th>分类</th><th style="text-align:right">金额</th><th></th></tr></thead><tbody>${selected.map((item) => `<tr><td><span class="transaction-title">${escapeHtml(item.summary)}</span>${item.note ? `<br><small class="muted">${escapeHtml(item.note)}</small>` : ""}</td><td>${shortDate(item.date)}</td><td><span class="tag coral">${escapeHtml(item.category.replaceAll(":", " · "))}</span>${item.budget_excluded ? ` <span class="tag amber">预算外</span>` : ""}</td><td class="money">${money(item.amount)}</td><td><div class="row-actions"><button data-edit-item="transaction" data-item-id="${escapeHtml(item.id)}">编辑</button><button class="danger" data-delete-item="transaction" data-item-id="${escapeHtml(item.id)}">删除</button></div></td></tr>`).join("") || `<tr><td colspan="5" class="muted">这个月还没有消费记录</td></tr>`}</tbody></table></section><aside class="card"><div class="card-head"><div class="card-title"><p>${money(state.data.summary.monthSpend)}</p><h3>分类占比</h3></div><div class="segmented"><button class="${state.categoryView === "parent" ? "active" : ""}" data-category-view="parent">一级</button><button class="${state.categoryView === "detail" ? "active" : ""}" data-category-view="detail">明细</button></div></div>${categoryPie(state.categoryView === "detail")}${categoryRows(12, state.categoryView === "detail")}</aside></div>`;
}

function renderDiary() {
  return `<div class="view-header"><div><h2>日记</h2><p>原样保存的生活片段，共 ${state.data.diary.length} 篇</p></div><button class="primary-button" data-open-composer="diary">${icon("plus")}写日记</button></div><div class="diary-page-list">${state.data.diary.map((item) => `<article class="card diary-page-entry"><header><time>${shortDate(item.date, true)}</time><div class="entry-head-actions"><div>${item.tags.map((tag) => `<span class="tag">${escapeHtml(tag)}</span>`).join(" ")}</div><div class="row-actions"><button data-edit-item="diary" data-item-id="${escapeHtml(item.id)}">编辑</button><button class="danger" data-delete-item="diary" data-item-id="${escapeHtml(item.id)}">删除</button></div></div></header><p>${escapeHtml(item.text)}</p></article>`).join("") || `<div class="empty-state"><p>还没有日记</p></div>`}</div>`;
}

function renderTodos() {
  const groups = { overdue: [], today: [], upcoming: [], undated: [] };
  state.data.todos.forEach((item) => groups[todoDueState(item).key].push(item));
  const section = (key, eyebrow, title, empty) => groups[key].length ? `<section class="card todo-section ${key}"><div class="card-head"><div class="card-title"><p>${eyebrow}</p><h3>${title}</h3></div><span class="section-count">${groups[key].length} 项</span></div><div class="todo-page-list">${todoRows(groups[key], null, { reschedule: key === "overdue", empty })}</div></section>` : "";
  return `<div class="view-header"><div><h2>待办清单</h2><p>${state.data.todos.length} 项进行中，逾期任务会优先显示</p></div><button class="primary-button" data-open-composer="todo">${icon("plus")}添加待办</button></div><div class="todo-sections">${section("overdue", "需要处理", "已逾期", "没有逾期待办")}${section("today", "今天", "今天截止", "今天没有截止任务")}${section("upcoming", "接下来", "即将截止", "近期没有截止任务")}${section("undated", "随时可做", "无截止日期", "没有无日期待办")}<section class="card completed-card"><div class="card-head"><div class="card-title"><p>最近完成</p><h3>可以撤销误操作</h3></div></div><div class="todo-page-list">${completedTodoRows()}</div></section></div>`;
}
...
function render() {
  ...
  const renderers = { overview: renderOverview, calendar: renderCalendar, expenses: renderExpenses, diary: renderDiary, todos: renderTodos, settings: renderSettings };
  ...
  try {
    state.settings = await request("/api/settings", { method: "PUT", body: JSON.stringify({
      ...
      backup: {
        ...
        includeSecrets: form.elements.includeSecrets.checked,
